...
export const uploadClient = createAlova({
  ...
  beforeRequest: method => {
    ...
    console.log(`[Upload Request] ${method.type.toUpperCase()} ${method.url}`, 'FormData in method.data');
  },
  ...
  responded: {
    onSuccess: async (response: Response, method) => {
      if (!response.ok) {
        ...
        const errorMessage = errorData?.message || errorData?.msg || response.statusText || `上传HTTP错误 ${response.status}`;
